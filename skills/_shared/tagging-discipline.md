...
Every PKIM record (EV, KN, CL, RL) **must** end up with a complete slash-namespaced tag set applied via `DTWriter.set_tags` before the skill returns success. The rule is non-negotiable. This document is the canonical reference linked from every skill that creates or transitions a record.
...
- Use `DTWriter.set_tags(uuid, tag_list)`. Do not stuff tags into custom metadata.
